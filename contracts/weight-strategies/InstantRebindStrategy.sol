...
contract InstantRebindStrategy is PoolManagement, WeightValueAbstract {
  ...
  /*** OWNER'S SETTERS ***/
  function setPoolRegistry(address _curvePoolRegistry) external {
    curvePoolRegistry = ICurvePoolRegistry(_curvePoolRegistry);
  }
      ...
      if (cfg.newBalance <= cfg.oldBalance) {
        ...
        uint256 usdcIn =
          (ICurvePoolRegistry(curvePoolRegistry).get_virtual_price_from_lp_token(IVault(cfg.token).token()) *
            crvAmount) / 1e18;
        //        uint256 usdcIn = ICurveDepositor(vc.depositor).calc_withdraw_one_coin(crvAmount, int128(vc.usdcIndex));
    ...
    returns (RebindConfig[] memory configs)
  {
    ...
    for (uint256 si = 0; si < len; si++) {
      ...
      configs[si] = RebindConfig(
        ...
        oldBalances[oi],
        bdiv((totalUSDCPool * newTokenValuesUSDC[oi]) / totalValueUSDC, bdiv(poolUSDCBalances[oi], oldBalances[oi]))
      );
    }
  }
