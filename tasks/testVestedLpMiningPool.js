...
task('test-vested-lp-mining-pool', 'Test VestedLpMining Pool').setAction(async (__, { ethers }) => {
  ...
  const testAddress = '0x3cc02dced7fc1272ecce381b69da9848d6332f04';
  ...
  const lpAddress = '0x9ba60ba98413a60db4c651d4afe5c937bbd8044b';
  ...
  await lpMining.add('1', lpAddress, '1', false, '385000000000000000000000000000', '385000000000000000000000000000', '14000000000', '140000000000', {from: admin});
  console.log('boostRate', boostRate);
  console.log('_lpBoostMinRatio', ether('0.39'));
  console.log('_lpBoostMaxRatio', ether('3.9'));
  const depositLp = ether('1000');
  const depositCvp = ether('140');
  const minCvpBalanceToBoost = await callContract(lpMining, 'cvpBalanceToBoost', [depositLp, lpAddress, true]);
  const maxCvpBalanceToBoost = await callContract(lpMining, 'cvpBalanceToBoost', [depositLp, lpAddress, false]);
