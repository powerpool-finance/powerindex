...
describe('PiBPoolController', () => {
    ...
    it('should correctly vote for CRV voting', async () => {
        const CRV = await MockERC20.new('CRV', 'CRV', ether('1000000'));
        const CrvVoting = contract({
            abi: fs.readFileSync('contracts/test/CrvVotingAbi.json'),
            bytecode: fs.readFileSync('contracts/test/CrvVoting').replace('d533a949740bb3306d119cc777fa900ba034cd52', CRV.address.replace('0x', ''))
        });
        CrvVoting.setProvider(web3.currentProvider);

        const router = await PiSimpleRouter.new();
        const wrapper = await WrappedPiErc20.new(CRV.address, router.address, 'WToken', 'WTKN');
        const poolRestrictions = await PoolRestrictions.new();
        const router2 = await PiRouter.new(poolRestrictions.address);
