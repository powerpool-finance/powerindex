...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    ...
    beforeEach(async () => {
        ...
        this.prepareReservoir = async function() {
            supply = await this.sushi.totalSupply();
        ...
        this.checkSushiSpent = async function(spentValue) {
            const reservoirBalance = await this.sushi.balanceOf(this.reservoir.address);
            assert.equal(toBN(supply).sub(toBN(reservoirBalance)).toString(10), toBN(spentValue).toString(10))
        };
    });

    it('should set correct state variables', async () => {
        this.chef = await MasterChef.new(this.sushi.address, this.reservoir.address, dev, '1000', '0', '1000', { from: alice });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.chef = await MasterChef.new(this.sushi.address, this.reservoir.address, dev, '100', '100', '1000', { from: alice });
        ...
        it('should not distribute SUSHIs if no one deposit', async () => {
            ...
            this.chef = await MasterChef.new(this.sushi.address, this.reservoir.address, dev, '100', '200', '1000', { from: alice });
            await this.prepareReservoir();
            await this.chef.add('100', this.lp.address, true);
            await this.lp.approve(this.chef.address, '1000', { from: bob });
            await time.advanceBlockTo('199');
            assert.equal((await this.sushi.balanceOf(this.reservoir.address)).toString(10), supply.toString(10));
            ...
            await this.checkSushiSpent('11000');
        ...
        it('should distribute SUSHIs properly for each staker', async () => {
            ...
            this.chef = await MasterChef.new(this.sushi.address, this.reservoir.address, dev, '100', '300', '1000', { from: alice });
            await this.prepareReservoir();
            await this.chef.add('100', this.lp.address, true);
            ...
            await this.checkSushiSpent('22000');
            assert.equal((await this.sushi.balanceOf(alice)).valueOf(), '5666');
            ...
            await this.checkSushiSpent('55000');
        ...
        it('should give proper SUSHIs allocation to each pool', async () => {
            ...
            this.chef = await MasterChef.new(this.sushi.address, this.reservoir.address, dev, '100', '400', '1000', { from: alice });
            await this.prepareReservoir();
            await this.lp.approve(this.chef.address, '1000', { from: alice });
        ...
        it('should stop giving bonus SUSHIs after the bonus period ends', async () => {
            ...
            this.chef = await MasterChef.new(this.sushi.address, this.reservoir.address, dev, '100', '500', '600', { from: alice });
            await this.prepareReservoir();
            await this.lp.approve(this.chef.address, '1000', { from: alice });
