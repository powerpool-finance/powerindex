...
describe('IndicesSupplyRedeemZap', () => {
  ...
  describe.skip('Supply and Redeem of VAULT pool', () => {
    ...
    beforeEach(async () => {
      ...
      const vaultRegistry = await MockCurvePoolRegistry.new();
      for (let i = 0; i < vaultsData.length; i++) {
        ...
        if (v.config.amountsLength === 2) {
          depositor = await MockCurveDepositor2.new(lpToken.address, usdc.address, v.config.usdcIndex, szabo(v.usdcToLpRate));
        } else if (v.config.amountsLength === 3) {
          depositor = await MockCurveDepositor3.new(lpToken.address, usdc.address, v.config.usdcIndex, szabo(v.usdcToLpRate));
        } else if (v.config.amountsLength === 4) {
          depositor = await MockCurveDepositor4.new(lpToken.address, usdc.address, v.config.usdcIndex, szabo(v.usdcToLpRate));
        }
